...
int main(){
  ...
  int delay=10;
  int alarm=1;
  char password[6]={'1','2','3','A','B','C'}; //default value for password
start:
  LCD_start();
  ...
  while(1)
  {
    ...
    while(press) //if a key is pressed
    {
      ...
      { if(key=='*'&&i==0)
      {
        configurationMenu(password,&delay,&alarm);
        goto start;
      }
      else{
        if(password[i]!=key)
    ...
    if(correct)
    {
      ...
      delayMs(delay*1000);//wait for 20 secs and then automtically lock the door
