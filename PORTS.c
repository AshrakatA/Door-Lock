...
void configurationMenu(char* password, int* delay,int* alarm){ 
  LCD_start();
  LCD_puts("  Configuration  ");
  LCD_command(0xc0);
  LCD_puts("      List      "); 
  delayMs(2000);
  LCD_command(0x01);
  LCD_puts("1-PW 2-DoorTime"); // set new password or change the duration of openning the door
  LCD_command(0xc0);
  LCD_puts("3-Alarm 4-Back"); //set alarm on or off or back to the start to open the door
  systick(100);
  char choice;
  int press=1;
  char key;
  ...
  case '2':
    LCD_start();
    LCD_puts("Set Time"); // set alarm on or off
    LCD_command(0xc0);
    char temp[]="";
    for(int i =0 ; i<2;i++)
    {
    ...
    *delay=atoi(temp);
    LCD_command(0x01);
    LCD_puts("Time Changed");
    delayMs(2000);
    break;
    
    case '3':
    LCD_start();
    LCD_puts("1-ON 2-Off"); // set alarm on or off
    LCD_command(0xc0);
    char k=listener();
    LCD_data(k);
    switch (k) {
